...
public class ReadInputStreamTask extends SwingWorker<Long, Map<Integer, String>>
{
	...
	// -- Instance Variable(s) --------------------------------------------------------
	// --------------------------------------------------------------------------------
	private IntervalPropertyChangeListener				intervalPCL;
	...
	// -- Public Method(s) ------------------------------------------------------------
	// --------------------------------------------------------------------------------
	public ReadInputStreamTask(InputStream in, final long available, byte[] buffer)
	{
		this.in = in;
		this.available = available;
		this.buffer = buffer;
		this.intervalPCL = new MyIntervalPropertyChangeListener(executor, 800L, TimeUnit.MILLISECONDS);
	...
	protected Long doInBackground() throws Exception
	{
		...
		while ((bytesRead = in.read(buffer)) > -1)
		{
			...
			intervalPCL.propertyChange(new PropertyChangeEvent(this, "totalBytesRead", null, totalBytesRead));
	...
	// --------------------------------------------------------------------------------
	private class MyIntervalPropertyChangeListener extends IntervalPropertyChangeListener
	{
		private long	lastTimeStamp;
		private long	lastTotalBytesRead;

		public MyIntervalPropertyChangeListener(ScheduledThreadPoolExecutor executor, long l, TimeUnit milliseconds)
		{
			super(executor, l, milliseconds);
		}
		...
		private void publishMap(long totalBytesRead)
		{
			...
			if (lastTimeStamp > 0)
			{
				speed = ProgressUtil.speed(totalBytesRead - lastTotalBytesRead, timeStamp - lastTimeStamp);
			...
			String string = String.format("%s/%s (ETA %s @ %s/s)", new ByteValue(totalBytesRead), new ByteValue(available), new TimeValue(
					remainingSecs * 1000), new ByteValue((int) speed));
