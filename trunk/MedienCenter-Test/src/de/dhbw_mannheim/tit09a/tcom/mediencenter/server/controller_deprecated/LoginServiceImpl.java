...
public class LoginServiceImpl extends ProxyUtil implements LoginService
{
	...
	@Override
	public synchronized Session login(String login, String pw, ClientCallback callback) throws RemoteException, IllegalAccessException
	{
		if (callback != null)
			callback.callback("Login is in progress ...");
		...
		if (sessionId != null)
		{
			...
			System.out.printf("Logged in %s. Now there are %d users online.%n", login, userSessions.size());
			if (callback != null)
				callback.callback("Successfully logged in.");
	...
	public synchronized boolean logout(Session session) throws RemoteException
	{
		...
		System.out.printf("Logged out %s. Now there are %d users online.%n", session.getLogin(), userSessions.size());
