package de.dhbw_mannheim.tit09a.tcom.mediencenter.test.authentication;
...
public class PasswordManager
{
    ...
    /**
     * Authenticates the user with a given login and password If password and/or login is null then always
     * returns false. If the user does not exist in the database returns false.
     * 
     * @param con
     ...
     * @throws SQLException
     *             If the database is inconsistent or unavailable ( (Two users with the same login, salt or
     *             digested password altered etc.)
     * @throws NoSuchAlgorithmException
     *             If the algorithm SHA-1 is not supported by the JVM
     */
    public boolean authenticate(Connection con, String login, String password) throws SQLException,
     ...
     */
    public boolean createUser(Connection con, String login, String password) throws SQLException,
	...
	try
	{
	    ...
	    {
		if(DBUtil.entryExists(con, "CREDENTIAL", "LOGIN", login))
		{
		    throw new IllegalArgumentException("Login existiert bereits: "+login);
		}
		...
		System.out.println(sDigest + "("+sDigest.length()+")");
		System.out.println(sSalt + "("+sSalt.length()+")");
     ...
     * @throws UnsupportedEncodingException
     *             If the Characater Encoding is not supported
     */
	...
	try
	{
	    ...
	    st.execute("CREATE TABLE CREDENTIAL (LOGIN VARCHAR(32) PRIMARY KEY, PASSWORD VARCHAR(88) NOT NULL, SALT VARCHAR(32) NOT NULL)");
	}
	finally
	{
	    close(st);
