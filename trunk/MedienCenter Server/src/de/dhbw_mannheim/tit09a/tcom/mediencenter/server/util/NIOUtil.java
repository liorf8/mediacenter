...

{
	...
	public static void rename(Path src, String newName) throws IOException
	{
		...
		{
			// TODO:
			throw new IllegalArgumentException(e.toString() + " -- AUTORENAME not supported yet!");
		}
	}
	...
	// --------------------------------------------------------------------------------
	public static int copyMove(Path src, Path targetDir, boolean replace, boolean move) throws IOException
	{
		if (!Files.exists(targetDir, LinkOption.NOFOLLOW_LINKS))
			throw new NoSuchFileException(targetDir.toString());
