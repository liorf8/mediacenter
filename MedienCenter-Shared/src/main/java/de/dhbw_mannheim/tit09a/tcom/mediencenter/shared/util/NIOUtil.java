...

{
	...
	public static String probeContentType(Path path)
	{
		...
		{
			System.err.println("Could not probe content type of " + path + " due to:");
			e.printStackTrace();
			return null;
		}
	}
	...
	public static String probeContentType(File file)
	{
		...
		{
			System.err.println("Could not probe content type of " + file + " due to:");
			e.printStackTrace();
			return null;
		}
	}
	...
	public static Path autoRename(Path path)
	{
		...
		while (Files.exists(path, LinkOption.NOFOLLOW_LINKS))
		{
			if (name.indexOf('.') > 0) // has file suffix (e.g. *.jpg)
				path = Paths.get(name.substring(0, name.lastIndexOf('.')) + " (" + (i++) + ")" + name.substring(name.lastIndexOf('.'), name.length()));
			else
