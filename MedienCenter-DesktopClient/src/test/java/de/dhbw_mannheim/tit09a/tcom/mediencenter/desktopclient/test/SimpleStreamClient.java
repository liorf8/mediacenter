...
public class SimpleStreamClient
{
	...
	private static StreamPlayer				remotePlayer	= null;

	private static final String				protocol		= "rtp";
	...
	private static void initSimon() throws UnknownHostException, LookupFailedException, EstablishConnectionFailed
	{
		...
		remotePlayer = session.getStreamPlayer();
		TranscodeOptions transcodeOptions = new TranscodeOptions("h264", null, 1024, 0, false, false);
		streamPath = remotePlayer.setAndGetStreamTarget(protocol, port, transcodeOptions);
	}
	...
	{
		final FileInfoTree_alt tree = new FileInfoTree_alt(session);
		tree.addTreeSelectionListener(new TreeSelectionListener()
		{
			...
			public void valueChanged(TreeSelectionEvent evt)
			{
				...
				if (fi != null)
				{
					...
					InfoPlayer infoPlayer = session.getInfoPlayer();
					try
					{
