...
public class ResetPwAction extends AbstractSwingWorkerAction
{
	...
	private class RegisterWorker extends AbstractTaskPanelSwingWorker
	{
		...
		protected void work() throws Exception
		{
			...
			int returnValue = JOptionPane.showConfirmDialog(loginPanel,
			...
			if (returnValue == JOptionPane.YES_OPTION)
			{
				...
				setProgress(100);
				JOptionPane.showMessageDialog(loginPanel, "Reset password successful", "Reset password for " + login
						+ ". Please ask the server admin for the new one.", JOptionPane.INFORMATION_MESSAGE);
			}
			else
			{
				this.cancel(false);
